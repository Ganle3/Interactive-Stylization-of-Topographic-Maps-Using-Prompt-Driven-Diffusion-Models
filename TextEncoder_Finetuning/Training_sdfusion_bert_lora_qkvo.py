...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs\sdfusion_bert_lora_qkv_color"
...
# cover cross-attn Q/K/V + out_proj（to_out[0]）
def lora_attn_qkvo(unet, r=8):
    n = 0
    for m in unet.modules():
        for attr in ["to_q", "to_k", "to_v"]:
            lin = getattr(m, attr, None)
            if isinstance(lin, nn.Linear):
                setattr(m, attr, LoRALinear(lin, r=r)); n += 1
        # to_out: Sequential([Linear, Dropout])
        if hasattr(m, "to_out") and hasattr(m.to_out, "__getitem__"):
            lin = m.to_out[0]
            if isinstance(lin, nn.Linear):
                m.to_out[0] = LoRALinear(lin, r=r); n += 1
    return n
    ...
    for m in unet.modules():
        ...
        for attr in ("to_q", "to_k", "to_v"):
            ...
            if isinstance(lin, LoRALinear):
                dev = lin.base.weight.device
                dty = lin.base.weight.dtype
                lin.A.to(device=dev, dtype=dty)
                lin.B.to(device=dev, dtype=dty)
        # check to_out[0]
        if hasattr(m, "to_out") and hasattr(m.to_out, "__getitem__"):
            lin = m.to_out[0]
            if isinstance(lin, LoRALinear):
...
wrapped = lora_attn_qkvo(model.model.diffusion_model, r=LORA_R)
...
#         params.append(p)
q_out_params = [p for n,p in model.model.diffusion_model.named_parameters()
                if (p.requires_grad and (".A.weight" in n or ".B.weight" in n) and
                    (".to_q." in n or ".to_out." in n or ".to_q" in n or ".to_out" in n))]
kv_params = [p for n,p in model.model.diffusion_model.named_parameters()
             if (p.requires_grad and (".A.weight" in n or ".B.weight" in n) and
                 not (".to_q." in n or ".to_out." in n or ".to_q" in n or ".to_out" in n))]

param_groups = [
    {"params": [p for n,p in textenc.hf_bert.named_parameters() if p.requires_grad], "lr": 5e-5, "weight_decay": 0.01},
    {"params": kv_params, "lr": 1e-5, "weight_decay": 0.01},
    {"params": q_out_params, "lr": 1e-6, "weight_decay": 0.01},
]
    ...
    for batch in dl:
        ...
        if global_step % VAL_EVERY == 0:
            ...
            if global_step % SAVE_IMG_EVERY == 0:
                # take = min(val_batch["seg"].size(0), 2)
                # seg_a = val_batch["seg"][:take]
                # gt_a = val_batch["gt"][:take]
                # prom_a = val_batch["prompt"][:take]
                model.eval(); textenc.eval()
                with torch.no_grad():
                    vb = next(iter(dl_val))
                    take = min(vb["seg"].size(0), 2)
                    seg_a = vb["seg"][:take]; gt_a = vb["gt"][:take]; prom_a = vb["prompt"][:take]

                    preds_a = sample_preview(model, textenc,
                                             {"seg": seg_a, "gt": gt_a, "prompt": prom_a},
                                             device, steps=30, scale=5.0, eta=0.0, seed=1234,
                                             use_text=True, disable_lora=False)
                    preds_b = sample_preview(model, textenc,
                                             {"seg": seg_a, "gt": gt_a, "prompt": [""] * take},
                                             device, steps=30, scale=1.0, eta=0.0, seed=1234,
                                             use_text=False, disable_lora=False)
                    preds_b0 = sample_preview(model, textenc,
                                              {"seg": seg_a, "gt": gt_a, "prompt": [""] * take},
                                              device, steps=30, scale=1.0, eta=0.0, seed=1234,
                                              use_text=False, disable_lora=True)

                    out_png = os.path.join(OUTDIR, f"vis/ab_{global_step:06d}.png")
                    save_ab_grid(seg_a, preds_a, preds_b, gt_a, out_png, prompts_a=prom_a, pred_b0=preds_b0)
                    print(f"   -> saved A/B/B0 preview to {out_png}")
