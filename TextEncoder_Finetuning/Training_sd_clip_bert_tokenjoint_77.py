...
from token_utils import build_cond_uc_safe_pad, _clip_ctx_and_after_eos
...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs\sdfusion_bert_ll_ofsp_color\DEEPERadpter_LoRA_joint_tokens77"
...
VAL_EVERY=100
TEXT_DROP_PROB = 0.1   # 20% empty prompt possibility in training stage to force model learn to use text
device = "cuda" if torch.cuda.is_available() else "cpu"
os.makedirs(OUTDIR, exist_ok=True)

# introduce tools from other script
from data_utils import stratified_split_ratio, PairsJSONLDataset
from vis_metrics import MeterEMA, validate_step, validate_image_metrics, preview_three_modes, save_abc_grid
...
class LoRALinear(nn.Module):
    def __init__(self, base: nn.Linear, r=16, alpha=1.5):
        super().__init__()
        self.base = base
...
if USE_LORA and len(lora_params) > 0:
    param_groups.append({"params": lora_params, "lr": 3e-5, "weight_decay": 0.0})
...
while global_step < STEPS:
    for batch in dl:
        ...
        if global_step == 1:
            ...
            # 1) 确认 start 分布（BERT 写入起点大多在 4~10 左右）
            clip_ctx, start = _clip_ctx_and_after_eos(model, ["map in swisstopo style"] * 4, device)
            print("[dbg] first write positions:", start.tolist())
        ...
        # 预热：前 500 步线性放大到 1.0
        def get_text_schedule(step):
            # γ：BERT 增量强度，400 步拉到 0.9，800 步拉到 1.0
            if step < 400:
                gamma = 0.6 + 0.3 * (step / 400.0)  # 0.6 -> 0.9
            elif step < 800:
                gamma = 0.9 + 0.1 * ((step - 400) / 400.0)  # 0.9 -> 1.0
            else:
                gamma = 1.0

            # w_clip：CLIP 基座占比，800 步降到 0.4
            if step < 800:
                w_clip = 1.0 - 0.6 * (step / 800.0)  # 1.0 -> 0.4
            else:
                w_clip = 0.4

            uc_mix = 0.0  # 取消软 CFG 混入，差分只来自 BERT
            return gamma, w_clip, uc_mix


        gamma, w_clip, uc_mix = get_text_schedule(global_step)
        ...
        with autocast(device_type='cuda', enabled=(device == "cuda"), dtype=torch.float16):
            ...
            cond, uc = build_cond_uc_safe_pad(
                ...
                gamma=gamma, w_clip=w_clip, bert_align=bert_align,
            )
            # # 2) 确认 cond/uc 的 shape 都是 [B,77,768]，且 cond 与 uc 在 [start:] 区间确实不同
            # print(cond["c_crossattn"][0].shape, uc["c_crossattn"][0].shape)
        ...
        if global_step % 100 == 0:
            ...
            print(f"[sched] step={global_step} gamma={gamma:.2f} w_clip={w_clip:.2f} uc_mix={uc_mix:.2f}")
        ...
        if global_step % VAL_EVERY == 0:
            ...
            if global_step % SAVE_IMG_EVERY == 0:
                ...
                seg_a = batch["seg"][:take].detach() # 在preview three modes里转-1，1了
                gt_a = batch["gt"][:take].detach()
                prom_a = batch["prompt"][:take]

                preds_main, preds_bert, preds_clipw = preview_three_modes(
                    model, textenc, seg_a, prom_a, device,
                    steps=25, scale=4.0, eta=0.0, seed=1234,
                    clip_style_text="map in swisstopo style",
                    gamma=1.0, w_clip=0.4,  # 可按需要调
                    bert_align=bert_align,  # 若没有就传 None
                    start_idx=8
                )

                out_png = os.path.join(OUTDIR, f"vis/train_abc_{global_step:06d}.png")
                save_abc_grid(seg_a, preds_main, preds_bert, preds_clipw, gt_a, out_png)
                print(f"   -> saved TRAIN A/B/C preview to {out_png}")
