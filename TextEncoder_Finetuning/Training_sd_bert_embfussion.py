...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs\sdfusion_bert_lastlayer_lora_txtembfus_color"
...
USE_LORA = True
LORA_R=2     # lr is setting in later before build opt
VAL_EVERY=100
...
def build_text_encoder(n_embed=768, n_layer=12, max_len=77, device="cuda", use_tokenizer=True):
    ...
    # freeze all parameters and only release the last layer FFN
    for p in hf_bert.parameters():
        p.requires_grad = False

    # last layer: output.dense + LayerNorm
    last_layer = hf_bert.encoder.layer[-1]

    for p in last_layer.output.dense.parameters():
        p.requires_grad = True

    for p in last_layer.output.LayerNorm.parameters():
        p.requires_grad = True

    # later option: intermediate.dense unfrozen
    for p in last_layer.intermediate.dense.parameters():
        p.requires_grad = True

    print("[Unfrozen BERT params]:",
          sum(p.numel() for p in hf_bert.parameters() if p.requires_grad))
    ...
    # put tokens to device and inject to transformer
    def encode_on_device(prompts):
        enc = tokenizer(
            prompts, return_tensors='pt',
            padding="max_length", truncation=True, max_length=max_len  # fixed 77
        )
...
textenc.train()
# combined CLIP and BERT emb first
bert_adapter = nn.Linear(768, 768, bias=False)
nn.init.zeros_(bert_adapter.weight)
beta_param = nn.Parameter(torch.tensor(-1.5))  # sigmoid(-1.5)≈0.18
bert_adapter = bert_adapter.to(device=device, dtype=torch.float32)
beta_param = nn.Parameter(beta_param.to(device=device, dtype=torch.float32))
# only fine-tuning: textenc pooler MLP
# bert_ffn_params = [p for n,p in textenc.hf_bert.named_parameters() if p.requires_grad]
lora_params = []   # if have any later, append it

param_groups = []
# if bert_ffn_params:
#     param_groups.append({"params": bert_ffn_params, "lr": 3e-5, "weight_decay": 0.01})
if lora_params:
    param_groups.append({"params": lora_params, "lr": 2e-4, "weight_decay": 0.0})

# opt = torch.optim.AdamW(param_groups, betas=(0.9, 0.999))
# param_groups = []
bert_ffn_params = [p for n,p in textenc.hf_bert.named_parameters() if p.requires_grad]
if bert_ffn_params:
    param_groups.append({"params": bert_ffn_params, "lr": 2e-5, "weight_decay": 0.01})

# new parameters for fusion two text emb
param_groups.append({"params": list(bert_adapter.parameters()) + [beta_param], "lr": 2e-5, "weight_decay": 0.0})

opt = torch.optim.AdamW(param_groups, betas=(0.9, 0.999), eps=1e-8)

scaler = GradScaler(enabled=(device=="cuda"))
print("Trainable HF-BERT names:", bert_ffn_params[:6], "…", len(bert_ffn_params))
...
while global_step < STEPS:
    for batch in dl:
        ...
        with autocast(device_type='cuda', enabled=(device == "cuda"), dtype=torch.float16):
            # keep Swisstopo original style embedding + my BERT embedding

            # fixed base prompt with original model's CLIP encoder to generate baseline embedding
            base_prompt = ["map in swisstopo style"] * len(prompts)
            with torch.no_grad():
                base_emb = model.get_learned_conditioning(base_prompt).to(device=device, dtype=torch.float16)

            # my new BERT embedding
            bert_emb = textenc.encode(prompts).to(device=device, dtype=torch.float16)
            # print("[DBG] base_emb:", base_emb.shape, "bert_emb:", bert_emb.shape)

            # add 2 emb: BERT control fine-tuning above original style embedding
            # base_emb: [B, 77, 768]
            # bert_emb: [B, 12, 768]
            target_len = base_emb.shape[1]
            B, L, C = bert_emb.shape

            if L < target_len:
                pad = torch.zeros(B, target_len - L, C, device=bert_emb.device, dtype=bert_emb.dtype)
                bert_emb = torch.cat([bert_emb, pad], dim=1)
            elif L > target_len:
                bert_emb = bert_emb[:, :target_len, :]

            # auto up/down scale bert_emb, to make 2 norm similar
            # norm_ratio = (base_emb.norm() / (bert_emb.norm() + 1e-6)).clamp(0.5, 2.0)
            # BETA_BERT = 0.5  # kind of hyperparameter
            # c_cross = base_emb + BETA_BERT * bert_emb * norm_ratio
            base_emb = F.layer_norm(base_emb, (768,))
            bert_emb = F.layer_norm(bert_emb, (768,))
            bert_aligned = bert_adapter(bert_emb)
            beta = torch.sigmoid(beta_param)     #trainable controler
            norm_ratio = (base_emb.norm() / (bert_emb.norm() + 1e-6)).clamp(0.5, 2.0)
            c_cross = base_emb + beta * bert_aligned * norm_ratio

            if global_step == 1:
                print("c_cross requires_grad?", c_cross.requires_grad)
                print("base_emb norm:", base_emb.norm().item(), "bert_emb norm:", bert_emb.norm().item())

            # build ControlNet condition
        ...
        if global_step % 100 == 0:
            ...
            base_norm = base_emb.norm().item()
            bert_norm = bert_emb.norm().item()
            bert_aligned_norm = bert_aligned.norm().item() if 'bert_aligned' in locals() else float('nan')
            print(f"beta={beta_val:.3f} | ||base_emb||={base_norm:.2f} ||bert_emb||={bert_norm:.2f} ||bert_aligned||={bert_aligned_norm:.2f}")
        ...
        if global_step % VAL_EVERY == 0:
            ...
            if global_step % SAVE_IMG_EVERY == 0:
                ...
                preds_a = sample_preview(model, textenc,
                                         ...
                                         device, steps=40, scale=7.5, eta=0.0, seed=1234)
...
torch.save(textenc.state_dict(), os.path.join(OUTDIR, "last_text_lastlayer_ffn_color.pt"))
lora_sd = {k: v for k, v in model.model.diffusion_model.state_dict().items()
           if k.endswith(".A.weight") or k.endswith(".B.weight")}
