...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs\sdfusion_bert_ll_ofsp_color\outonly"
...
def build_text_encoder(n_embed=768, n_layer=12, max_len=77, device="cuda", use_tokenizer=True):
    ...
    print("[Unfrozen BERT params]:", sum(p.numel() for p in hf_bert.parameters() if p.requires_grad))
    ...
    def encode_on_device(prompts):
        ...
        # x = out.last_hidden_state  # [B,77,768]
...
#         params.append(p)
param_groups = [
    {"params": [p for n,p in textenc.hf_bert.named_parameters() if p.requires_grad], "lr": 5e-6, "weight_decay": 0.01},
    {"params": [p for n,p in model.model.diffusion_model.named_parameters()
                if p.requires_grad and (".A.weight" in n or ".B.weight" in n)], "lr": 2e-6, "weight_decay": 0.01},
]
...
split_paths = stratified_split_ratio(
    JSONL, SPLIT_DIR, per_prompt_ratio=(0.1, 0.1, 0.8),     # 先判断过拟合训练小集
    ...
    for batch in dl:
        ...
        if global_step % VAL_EVERY == 0:
            ...
            img_metrics = validate_image_metrics(
                model, textenc, vb0, device, steps=15, scale=5.5, eta=0.0
            )
            ...
            if global_step % SAVE_IMG_EVERY == 0:
                ...
                preds_a = sample_preview(model, textenc,
                                         ...
                                         device, steps=15, scale=5.5, eta=0.0, seed=1234)
