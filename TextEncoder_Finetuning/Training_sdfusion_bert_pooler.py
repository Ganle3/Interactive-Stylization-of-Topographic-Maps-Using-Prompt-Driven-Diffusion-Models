...
SAVE_IMG_EVERY = 100    # steps for visualization
...
def build_text_encoder(n_embed=768, n_layer=12, max_len=77, device="cuda", use_tokenizer=True):
    ...
    #     def __init__(self, in_dim=768, hid=3072, out_dim=768):
    #         super().__init__()
    #         self.fc1 = nn.Linear(in_dim, hid, bias=True)
    #         self.act = nn.GELU()
    #         self.fc2 = nn.Linear(hid, out_dim, bias=True)
    #         self.ln = nn.LayerNorm(out_dim, eps=1e-12, elementwise_affine=True)
    #
    #     def forward(self, x_cls):
    #         y = self.fc2(self.act(self.fc1(x_cls)))
    #         return self.ln(y)

    class PoolerMLP(nn.Module):
        """
        CLS-based deep MLP pooler: [B,L,D] -> take [CLS] -> 768→3072→3072→768,
        GELU activations, optional dropout, final LayerNorm. No tanh.
        """

        def __init__(self, hidden_dim=768, mid_dim=3072, dropout=0.1, eps=1e-12):
            super().__init__()
            self.fc = nn.Sequential(
                ...
                nn.Linear(mid_dim, hidden_dim, bias=True),
            )
            self.norm = nn.LayerNorm(hidden_dim, eps=eps, elementwise_affine=True)
    ...
    hf_bert.pooler = PoolerMLP().to(device)

    for p in hf_bert.pooler.parameters():
    ...
    def encode_on_device(prompts):
        ...
        out = hf_bert(input_ids=input_ids, attention_mask=attn_mask)     # last_hidden_state
        tokens = out.last_hidden_state  # [B,L,768]
        cls_vec = tokens[:, 0, :]  # [B,768]
        pooled = hf_bert.pooler(cls_vec)  # [B,768]  (MLP+LN, without tanh)
        tokens = tokens + pooled.unsqueeze(1)  # ensure validated pooler
        return tokens  # [B, L, 768]
...
param_groups = [
    {"params": [p for n,p in textenc.hf_bert.named_parameters() if p.requires_grad],
...
while global_step < STEPS:
    for batch in dl:
        ...
        with autocast(device_type='cuda', enabled=(device == "cuda"), dtype=torch.float16):
            ...
            bert_emb = 1.5 * bert_emb * norm_ratio  # 1.5 upscale BERT control
            c_cross = base_emb + bert_emb
        ...
        if global_step % VAL_EVERY == 0:
            ...
            if score < best_score:
                best_score = score
                torch.save(textenc.state_dict(), os.path.join(OUTDIR, "best_text_lastpooler.pt"))
...
torch.save(textenc.state_dict(), os.path.join(OUTDIR, "last_text_lastpooler.pt"))
# lora_sd = {k: v for k, v in model.model.diffusion_model.state_dict().items()
#            if k.endswith(".A.weight") or k.endswith(".B.weight")}
