...
def mse_rgb(pred, gt):
    sse = torch.mean((pred - gt) ** 2).item()
    mse = math.sqrt(sse) / 255
    return mse
...
def color_histogram_l1_255(pred255, gt255, bins=32):
    pred = pred255.permute(1, 2, 0).cpu().numpy()
    gt   = gt255.permute(1, 2, 0).cpu().numpy()
...
for gi, grp in enumerate(groups):
    ...
    for batch in grp:
        ...
        pred = sample_preview(
            model, textenc,
            batch, device,
            steps=15, scale=7.5, eta=0.0, seed=1234
        ).clamp(-1,1)
