...
class ControlLoRA(ModelMixin, ConfigMixin):
    ...
    # def forward(self, x: torch.FloatTensor, context: torch.FloatTensor = None, return_dict: bool = True):
    #     """
    #     x: structural condition (segmentation map)
    #     context: text embedding from BERT (77x768)
    #     """
    #     lora_layer: ControlLoRACrossAttnProcessor
    #     orig_dtype = x.dtype
    #     dtype = self.conv_in.weight.dtype
    ...
    #         control_states = control_states.to(orig_dtype)
    #
    #         # 注入 control + text context
    #         for lora_layer in lora_layer_list:
    #             lora_layer.inject_control_states(control_states)
    #             if context is not None:
    #                 # 把文本 embedding 作为 encoder_hidden_states 传给 attention
    #                 lora_layer.cross_attention_dim = context.shape[-1]
    #         control_states_list.append(control_states)
    #
    #     if not return_dict:
    #         return tuple(control_states_list)
