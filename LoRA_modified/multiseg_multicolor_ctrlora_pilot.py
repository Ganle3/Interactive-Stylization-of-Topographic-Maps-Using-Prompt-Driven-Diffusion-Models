...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\batch_switch_ctrlora"
...
def build_text_encoder(n_embed=768, n_layer=12, max_len=77, device="cuda", use_tokenizer=True):
    ...
    hf_bert = BertModel.from_pretrained("bert-base-uncased").cpu()
    ...
    def encode_on_device(prompts):
        ...
        tokens = te.adapter(tokens.to(device))        # only layer to train
        return tokens
...
for n, p in model.named_parameters():
    ...
    elif "zero_convs" in n or "middle_block_out" in n:
        p.requires_grad = True
    elif "norm" in n:
        p.requires_grad = True
    else:
...
main_ctrl_params = [
    ...
    if p.requires_grad and ("zero_convs" in n or "middle_block_out" in n or "norm" in n)
]
print(f"[check] trainable main control params: {sum(p.numel() for p in main_ctrl_params)}")

param_groups = [
    {"params": adapter_params, "lr": 7e-5},
    {"params": lora_params,    "lr": 1e-4},
    {"params": main_ctrl_params, "lr": 1e-5},  # 主体 ControlNet 较小 lr
]
...
dl = DataLoader(ds_mixed, batch_size=group_size, shuffle=True, num_workers=0, pin_memory=False)
