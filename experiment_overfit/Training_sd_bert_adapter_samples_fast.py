...
ROOTDIR = r"D:\Junyhuang\Project2_Data\Training Data\One seg multiprompt"
JSONL   = os.path.join(ROOTDIR, "meta", "pairs.jsonl")
SPLIT_DIR = os.path.join(ROOTDIR, "meta_overfit_test_sample")
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\adapter_wider_lora_bc_of1"
...
from TextEncoder_Finetuning.data_utils import stratified_split_ratio, PairsJSONLDataset, RepeatPairsDataset
from TextEncoder_Finetuning.vis_metrics import MeterEMA, validate_step, save_concept_grid, sample_preview, to_vis
...
class SubsetStaticDataset(torch.utils.data.Dataset):
    """
    Wrap a base dataset (e.g. PairsJSONLDataset) but expose only a fixed subset of indices.
    We keep original items (seg, gt, prompt) so that validation / visualization
    can still access the ground truth images.
    """
    def __init__(self, base_dataset, indices):
        self.base = base_dataset
        self.indices = indices  # list of ints
...
class LatentCachedDataset(torch.utils.data.Dataset):
    """
    Wrap a base dataset (like your repeated dataset) and replace 'gt' with its VAE latent 'z'.
    We precompute z for every item once on GPU, then keep it on CPU for training reuse.
    During training we no longer call encode_first_stage(gt) every step.
    """
    def __init__(self, base_dataset, model, device):
        self.base = base_dataset
        self.device = device

        print("[cache] Pre-encoding VAE latents for training dataset...")
        self.cache = []

        # 我们一次性把所有样本 encode 成 latent z，然后放到 CPU 缓存里
        # 注意：我们是按 base_dataset 的 __len__ 遍历，所以包含重复（比如 repeat=100）
        with torch.no_grad():
            for idx in range(len(self.base)):
                item = self.base[idx]

                seg = item["seg"]          # [C,H,W], 0..1
                gt  = item["gt"]           # [C,H,W], 可能是 [-1,1] 或 0..1，下面统一
                prompt = item["prompt"]

                gt_t = gt.unsqueeze(0).to(device, dtype=torch.float32)

                # 确保范围是 [-1,1]，避免有的版本是 0..1
                if gt_t.min() >= 0.0 and gt_t.max() <= 1.0:
                    gt_t = gt_t * 2.0 - 1.0

                # encode to z latent
                z = model.get_first_stage_encoding(
                        model.encode_first_stage(gt_t)
                    ).to(dtype=torch.float16)  # [1,4,h/8,w/8]

                # 我们把 seg 和 prompt 原样存，z 存 half 精度在 CPU，节省显存
                self.cache.append({
                    "seg": seg.clone(),             # float32 [C,H,W], stays 0..1
                    "z":   z.squeeze(0).cpu(),      # float16 [4,h/8,w/8] on CPU
                    "prompt": prompt
                })

                if (idx+1) % 50 == 0 or idx == len(self.base)-1:
                    print(f"[cache] encoded {idx+1}/{len(self.base)}")

        print("[cache] Done pre-encoding all latents.")
    ...
    def __getitem__(self, idx):
        item = self.cache[idx]
        # 注意：这一步不把东西搬到 GPU。搬到 GPU 的动作留给训练循环，和原来一样。
        return {
            "seg": item["seg"],         # float32 [C,H,W], still 0..1
            "z":   item["z"],           # float16 [4,h/8,w/8], CPU
            "prompt": item["prompt"]
        }
...
def build_text_encoder(n_embed=768, n_layer=12, max_len=77, device="cuda", use_tokenizer=True):
    ...
    #     def __init__(self, dim=768, gate_init=0.0):
    #         super().__init__()
    ...
    #         self.ln = nn.LayerNorm(dim)
    #
    #         # deeper FFN: dim -> hidden -> hidden -> dim
    #         self.ff = nn.Sequential(
    #             nn.Linear(dim, hidden, bias=True),  # proj up
    #             nn.GELU(),
    #             nn.Linear(hidden, hidden, bias=True),  # extra depth
    #             nn.GELU(),
    #             nn.Linear(hidden, dim, bias=True)  # proj back
    #         )
    #
    #         nn.init.zeros_(self.ff[-1].weight)
    #         nn.init.zeros_(self.ff[-1].bias)
    ...
    #
    #     def forward(self, x):  # x: (B, L, 768)
    #         y = self.ff(self.ln(x))  # shape still (B, L, 768)
...
# def lora_kv(unet, r=16):
#     """
#     只在 decoder output_blocks 的 cross-attn (attn2) 挂 LoRA
#     这些层负责颜色/纹理生成，最受 prompt 影响
#     """
#     n = 0
#     for name, m in unet.named_modules():
#         if "output_blocks" in name and hasattr(m, "attn2"):
#             if hasattr(m.attn2, "to_k") and isinstance(m.attn2.to_k, nn.Linear):
#                 m.attn2.to_k = LoRALinear(m.attn2.to_k, r=r); n += 1
#             if hasattr(m.attn2, "to_v") and isinstance(m.attn2.to_v, nn.Linear):
#                 m.attn2.to_v = LoRALinear(m.attn2.to_v, r=r); n += 1
#     print(f"[LoRA] attached to {n} decoder cross-attn linear layers.")
#     return n
...
# === 构建优化器 param_groups ===
param_groups = [
    {"params": adapter_params + hf_params, "lr": 5e-5, "weight_decay": 0.00},  # 主要训 adapter
    {"params": lora_params,               "lr": 5e-5, "weight_decay": 0.00},  # LoRA
]
...
split_paths = stratified_split_ratio(
    JSONL, SPLIT_DIR, per_prompt_ratio=(1, 0, 0),
    min_per_split=(1, 0, 0), shuffle_seed=42, verbose=True
)  # 11*100
...
ds_val = ds_base
# ds_val_raw = PairsJSONLDataset(JSONL, ROOTDIR, image_size=SIZE, seg_mode="RGB")

# sanity check
print("[base] len:", len(ds_base))  # 应该是 1100
a0 = ds_base[0]
# print("[base] seg range:", float(a0["seg"].min()), float(a0["seg"].max()))
# print("[base] gt  range:", float(a0["gt"].min()),  float(a0["gt"].max()))

REPEAT = 100            # 11 × 100 = 1100 条
ds_train_raw = RepeatPairsDataset(ds_base, repeat=REPEAT)
# val_indices = list(range(11))
# ds_val = SubsetStaticDataset(ds_val_raw, val_indices)    # 验证就用原始 10 条
ds_train = LatentCachedDataset(ds_train_raw, model, device)

# 3. 用缓存后的 ds_train 来建 DataLoader
dl     = DataLoader(ds_train, batch_size=BATCH, shuffle=True, num_workers=0, pin_memory=True)
dl_val = DataLoader(ds_val,   batch_size=11, shuffle=False, num_workers=0, pin_memory=True)
...
# 固定1张样本的索引
# one = base_ds[112]     # sample

# REPEAT = 1000
# ds_train = RepeatDataset(one, repeat=REPEAT)
...
while global_step < STEPS:
    for batch in dl:
        ...
        # seg: [B,C,H,W] float32 0..1
        seg = batch["seg"].to(device=device, dtype=torch.float32)

        # z: [B,4,h/8,w/8] float16 on CPU -> move to GPU
        z = batch["z"].to(device=device, dtype=torch.float16)

        prompts = batch["prompt"]

        t = torch.randint(0, model.num_timesteps, (z.size(0),), device=device).long()

        noise = torch.randn_like(z, dtype=torch.float16)
        ...
        with autocast(device_type='cuda', enabled=(device=="cuda"), dtype=torch.float16):
            ...
            loss = (F.l1_loss(eps_hat.float(), noise.float()) +
                    0.5 * F.mse_loss(eps_hat.float(), noise.float()))
        ...
        if global_step % 200 == 0:
            ...
            l1 = F.l1_loss(eps_hat.float(), noise.float())
            mse = F.mse_loss(eps_hat.float(), noise.float())
            loss = l1 + 50 * mse
            print(f"[{global_step}] L1={l1.item():.4f}  MSE={mse.item():.4f}")
            with torch.no_grad():
                test_prompts = [
                ...
                e = textenc.encode(test_prompts)
                mean = e.mean().item()
                std = e.std().item()
                diff = (e[0] - e[1]).abs().mean().item()
                print(f"[{global_step}] emb mean={mean:.4f}, std={std:.4f}, Δpink-yellow={diff:.6f}")
            # for n, p in model.model.diffusion_model.named_parameters():
            #     if n.endswith("to_k.B.weight"):
            #         print(f"[live LoRA B] step {global_step} {n} grad? {p.grad is not None} "
            #               f"norm={p.data.norm().item():.6f} requires_grad={p.requires_grad}")
            #         break
            # for n, p in model.model.diffusion_model.named_parameters():
            #     if n.endswith("to_v.B.weight"):
            #         print(f"[live LoRA B] step {global_step} {n} grad? {p.grad is not None} "
            #               f"norm={p.data.norm().item():.6f} requires_grad={p.requires_grad}")
            #         break
        ...
        if global_step % VAL_EVERY == 0:
            ...
            if global_step % SAVE_IMG_EVERY == 0:
                ...
                with torch.no_grad():
                    # 拿到整套 10 条（batch=10）
                    for vb in dl_val:
                        seg_v = vb["seg"][0:1].to(device, torch.float32)  # 第0张 seg 即为固定那张
                        ...
                        save_concept_grid(seg_v, preds_list, gts_list, proms, out_png)
                        break  # 只可视化一组（10条）
                model.train();
                textenc.train()
