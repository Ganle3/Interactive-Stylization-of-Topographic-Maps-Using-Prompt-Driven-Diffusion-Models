...
ROOTDIR = r"D:\Junyhuang\Project2_Data\Training Data\Multi seg multiprompt"
JSONL   = os.path.join(ROOTDIR, "meta", "pairs.jsonl")
SPLIT_DIR = os.path.join(ROOTDIR, "meta_overfit_test_sample")
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\multi_seg\building_color_adapter1_lora10"
...
LORA_R=10     # lr is setting in later before build opt
VAL_EVERY=500
...
# === 构建优化器 param_groups ===
param_groups = [
    {"params": adapter_params + hf_params, "lr": 2e-5, "weight_decay": 0.01},  # 主要训 adapter
...
ds_val = Subset(ds_base, range(11))
# ds_val_raw = PairsJSONLDataset(JSONL, ROOTDIR, image_size=SIZE, seg_mode="RGB")

# sanity check
print("[base] len:", len(ds_base))  # 应该是几千
print("[base] len:", len(ds_val))   # 应该是 11
