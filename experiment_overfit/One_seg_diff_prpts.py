# one seg but targets: change buildings in one tile to different color
# -*- coding: utf-8 -*-
...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\one seg\multi_prompts_adapter2_lora16_lcc_of"
...
from TextEncoder_Finetuning.vis_metrics import MeterEMA, validate_step, save_concept_grid, sample_preview
...
# sanity check
print("[base] len:", len(ds_base))  # 应该是 11
...
dl_val = DataLoader(ds_val,   batch_size=5, shuffle=False, num_workers=0, pin_memory=True)
...
while global_step < STEPS:
    for batch in dl:
        ...
        with autocast(device_type='cuda', enabled=(device=="cuda"), dtype=torch.float16):
            ...
            latent_mean = eps_hat.mean(dim=[2, 3])  # [B, 4]
            contrast = latent_mean.std(dim=0).mean()  # 标准差越大，颜色差异越明显
            loss = F.mse_loss(eps_hat.float(), noise.float()) - 0.05 * contrast

            if global_step % 100 == 0:
                print(f"contrast={contrast.item():.4f}")
