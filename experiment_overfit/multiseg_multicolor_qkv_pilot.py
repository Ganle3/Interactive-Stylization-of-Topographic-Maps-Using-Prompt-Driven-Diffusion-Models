...
ROOTDIR = r"D:\Junyhuang\Project2_Data\Training Data\Sameseg_diffitem_24_4perseg"
JSONL   = os.path.join(ROOTDIR, "meta", "pairs_grouped.jsonl")
SPLIT_DIR = os.path.join(ROOTDIR, "meta_overfit_test_sample")
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\batch_switch_qkv\24prompts_4perseg"
LOSS_LOG_PATH = os.path.join(OUTDIR, "vis", "loss_log.json")
os.makedirs(os.path.join(OUTDIR, "vis"), exist_ok=True)

# Training hyperparameters
BATCH=8; SIZE=512; STEPS=60000; MAXLEN=77
LORA_R=16     # lr is setting in later before build opt
VAL_EVERY=1500
TEXT_DROP_PROB = 0.005   # empty prompt possibility in training stage to force model learn to use text
...
SAVE_IMG_EVERY = 1500    # steps for visualization
...
def build_text_encoder(n_embed=768, n_layer=12, max_len=77, device="cuda", use_tokenizer=True):
    ...
    class LastStateAdapter(nn.Module):
        def __init__(self, dim=768, hidden=3072):
            super().__init__()
...
hf_params = [p for p in textenc.hf_bert.parameters() if p.requires_grad]

lora_q_params = [
    p for n, p in model.model.diffusion_model.named_parameters()
    if ".to_q." in n and (".A.weight" in n or ".B.weight" in n)
]
lora_kv_params = [
    p for n, p in model.model.diffusion_model.named_parameters()
    if any(x in n for x in [".to_k.", ".to_v."]) and (".A.weight" in n or ".B.weight" in n)
]
...
print(f"[check] q-lora params:  {sum(p.numel() for p in lora_q_params)}")
print(f"[check] kv-lora params: {sum(p.numel() for p in lora_kv_params)}")

assert (len(adapter_params) + len(lora_q_params) + len(lora_kv_params)) > 0, \
    "No trainable params collected!"

param_groups = [
    {"params": adapter_params + hf_params, "lr": 7e-5, "weight_decay": 0.005},  # adapter
    {"params": lora_kv_params, "lr": 1e-4, "weight_decay": 0.001},             # K/V LoRA
    {"params": lora_q_params, "lr": 2e-5, "weight_decay": 0.001},                # Q LoRA
]
...
# dl_seg = DataLoader(ds_base, batch_size=BATCH, shuffle=False, num_workers=0, pin_memory=True)
...
while global_step < STEPS:
    for batch in dl:
        ...
        if global_step % VAL_EVERY == 0:
            ...
            lora_q_sd = {k: v for k, v in state_dict.items()
                         if ".to_q." in k and (k.endswith(".A.weight") or k.endswith(".B.weight"))}
            lora_kv_sd = {k: v for k, v in state_dict.items()
                          if (".to_k." in k or ".to_v." in k) and (k.endswith(".A.weight") or k.endswith(".B.weight"))}

            # 保存为两个独立文件，方便后续只加载某部分
            torch.save(lora_q_sd, os.path.join(OUTDIR, f"unet_loraQ_step{global_step:06d}.pt"))
            torch.save(lora_kv_sd, os.path.join(OUTDIR, f"unet_loraKV_step{global_step:06d}.pt"))
...
lora_kv_sd = {k: v for k, v in state_dict.items()
              if (".to_k." in k or ".to_v." in k) and (k.endswith(".A.weight") or k.endswith(".B.weight"))}
torch.save(lora_q_sd, os.path.join(OUTDIR, f"unet_loraQ_laststep.pt"))
torch.save(lora_kv_sd, os.path.join(OUTDIR, f"unet_loraKV_laststep.pt"))
print("done.")
