...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\pilot_adapter2_lora8_addffn_of"
...
def build_text_encoder(n_embed=768, n_layer=12, max_len=77, device="cuda", use_tokenizer=True):
    ...
    def encode_on_device(prompts):
        ...
        tokens = tokens.to(device=device, dtype=torch.float16)
        return tokens
...
# 额外注入：Feed-Forward 层的 LoRA（如 proj_out / ff.net.0.proj）
def lora_ff(unet, r=8):
    """
    Inject LoRA into FeedForward layers of BasicTransformerBlock inside SpatialTransformer.
    Covers ff.net[0].proj and ff.net[2].
    """
    from ldm.modules.attention import SpatialTransformer
    n = 0

    for name, module in unet.named_modules():
        # 只找 SpatialTransformer
        if not isinstance(module, SpatialTransformer):
            continue

        # 遍历其中的 BasicTransformerBlock
        for j, block in enumerate(module.transformer_blocks):
            if not hasattr(block, "ff"):
                continue

            # ff.net[0].proj
            if hasattr(block.ff.net[0], "proj") and isinstance(block.ff.net[0].proj, nn.Linear):
                old = block.ff.net[0].proj
                block.ff.net[0].proj = LoRALinear(old, r=r)
                n += 1
                print(f"[lora_ff] injected → {name}.transformer_blocks.{j}.ff.net.0.proj")

            # ff.net[2]
            if isinstance(block.ff.net[2], nn.Linear):
                old = block.ff.net[2]
                block.ff.net[2] = LoRALinear(old, r=r)
                n += 1
                print(f"[lora_ff] injected → {name}.transformer_blocks.{j}.ff.net.2")

    print(f"[lora_ff] injected {n} feed-forward linear layers with LoRA (r={r})")
    return n
...
wrapped_ff = lora_ff(model.model.diffusion_model, r=4)
print("LoRA FFN-wrapped linears:", wrapped_ff)
...
lora_params_attn = [p for n,p in model.model.diffusion_model.named_parameters()
                    if p.requires_grad and ("attn" in n) and (".A.weight" in n or ".B.weight" in n)]
lora_params_ffn = [p for n,p in model.model.diffusion_model.named_parameters()
                   if p.requires_grad and ("attn" not in n) and (".A.weight" in n or ".B.weight" in n)]
...
print(f"[check] attn LoRA params: {sum(p.numel() for p in lora_params_attn)}")
print(f"[check] ffn LoRA params:  {sum(p.numel() for p in lora_params_ffn)}")

# === param groups ===
param_groups = [
    {"params": adapter_params + hf_params, "lr": 7e-5, "weight_decay": 0.0},  # text adapter
    {"params": lora_params_attn, "lr": 1e-4, "weight_decay": 0.0},            # attention LoRA
    {"params": lora_params_ffn,  "lr": 3e-5, "weight_decay": 0.0},            # FFN LoRA（smaller lr）
]

# 至少要有一组非空
assert (len(adapter_params) + len(hf_params) + len(lora_params_attn)) > 0, "No trainable params collected!"
...
ds_val = Subset(ds_base, [8, 208, 408, 608, 808, 1008])
...
dl     = DataLoader(ds_base, batch_size=BATCH, shuffle=True, num_workers=0, pin_memory=True)
...
while global_step < STEPS:
    for batch in dl:
        ...
        if global_step % VAL_EVERY == 0:
            ...
            adapter_sd = textenc.adapter.state_dict()  # 只存 adapter 模块
            torch.save(adapter_sd, os.path.join(OUTDIR, f"textenc_adapter_step{global_step:06d}.pt"))
