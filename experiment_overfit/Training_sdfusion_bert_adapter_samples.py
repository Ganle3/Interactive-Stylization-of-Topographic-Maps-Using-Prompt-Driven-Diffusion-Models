...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\adapter_lora_bc_of"
...
# 固定数据对
ds_base = PairsJSONLDataset( train_jsonl, ROOTDIR, image_size=SIZE, seg_mode="RGB")

# sanity check
print("[base] len:", len(ds_base))  # 应该是 10
a0 = ds_base[0]
...
ds_train = RepeatPairsDataset(ds_base, repeat=REPEAT)
ds_val   = ds_base      # 验证就用原始 10 条
dl     = DataLoader(ds_train, batch_size=BATCH, shuffle=False, num_workers=0, pin_memory=True)
dl_val = DataLoader(ds_val,   batch_size=10,    shuffle=False, num_workers=0, pin_memory=True)
...
while global_step < STEPS:
    for batch in dl:
        ...
        if global_step % VAL_EVERY == 0:
            ...
            torch.save(textenc.state_dict(), os.path.join(OUTDIR, "current_textenc_adapter.pt"))
