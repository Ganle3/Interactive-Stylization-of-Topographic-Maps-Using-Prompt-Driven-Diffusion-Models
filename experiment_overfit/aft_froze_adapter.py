...
ROOTDIR = r"D:\Junyhuang\Project2_Data\Training Data\Sameseg_diffitem"
JSONL   = os.path.join(ROOTDIR, "meta", "pairs_grouped.jsonl")
SPLIT_DIR = os.path.join(ROOTDIR, "meta_overfit_test_sample")
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\aft_8000_lora16_bert11output"
LOSS_LOG_PATH = os.path.join(OUTDIR, "vis", "loss_log.json")
os.makedirs(os.path.join(OUTDIR, "vis"), exist_ok=True)

# Training hyperparameters
BATCH=6; SIZE=512; STEPS=15000; MAXLEN=77
LORA_R=16     # lr is setting in later before build opt
VAL_EVERY=500
...
# === 1. 初始化 Text Encoder 并加载 8000 步的 adapter ===
textenc = build_text_encoder(n_embed=768, n_layer=12, max_len=MAXLEN, device=device)

# 加载之前保存的 adapter checkpoint
ADAPTER_PT = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\textenc_adapter_step008000.pt"
print(f"[load] loading adapter weights from: {ADAPTER_PT}")
full_sd = torch.load(ADAPTER_PT, map_location=device)
# 只提取 adapter 层的参数
adapter_sd = {k.replace("adapter.", ""): v for k, v in full_sd.items() if k.startswith("adapter.")}
print(f"[load] loading adapter weights: {len(adapter_sd)} tensors")
missing, unexpected = textenc.adapter.load_state_dict(adapter_sd, strict=False)
print(f"[adapter load] missing={len(missing)}, unexpected={len(unexpected)}")
textenc.adapter.load_state_dict(adapter_sd, strict=True)

# 冻结 adapter，只训练 LoRA
for p in textenc.adapter.parameters():
    p.requires_grad = False
print("[freeze] adapter parameters frozen.")

disable_bert_dropout(textenc.hf_bert)
print("[debug] BERT dropout disabled.")
textenc.eval()  # adapter 已冻结，可设为 eval 模式（无dropout）

# 验证是不是语义僵死
for n, p in textenc.hf_bert.encoder.layer[11].output.named_parameters():
    p.requires_grad = True
print("[unfreeze] BERT layer[11].output params:",
      sum(p.numel() for p in textenc.hf_bert.encoder.layer[11].output.parameters()))


# === 2. 加载 UNet LoRA checkpoint (第 8000 步) ===
LORA_PT = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\unet_lora_step008000.pt"
print(f"[load] loading LoRA weights from: {LORA_PT}")
lora_sd = torch.load(LORA_PT, map_location=device)

# 手动部分加载：只复制匹配 shape 的前 min(r_old, r_new)
current_sd = model.model.diffusion_model.state_dict()
for k, v in lora_sd.items():
    if k in current_sd and current_sd[k].shape != v.shape:
        # shape 不同：取交集部分拷贝
        tgt = current_sd[k]
        min_rows = min(tgt.shape[0], v.shape[0])
        min_cols = min(tgt.shape[1], v.shape[1])
        print(f"[resize] {k}: ckpt {tuple(v.shape)} -> model {tuple(tgt.shape)}  (copying {min_rows}x{min_cols})")
        tgt[:min_rows, :min_cols] = v[:min_rows, :min_cols]
        current_sd[k] = tgt
    elif k in current_sd:
        current_sd[k] = v

# 加载合并后的 state_dict
model.model.diffusion_model.load_state_dict(current_sd, strict=False)
print("[load] UNet LoRA weights loaded with partial rank adaptation.")
# 重新确认设备匹配
ensure_lora_to_device(model.model.diffusion_model, device)

# === 3. 构建优化器，只训练 LoRA ===
adapter_params = []  # 已冻结
hf_params      = []  # 全冻结 # 开放最后一层
...
bert_output_params = [p for p in textenc.hf_bert.encoder.layer[11].output.parameters()
                      if p.requires_grad]
print(f"[check] trainable bert params: {sum(p.numel() for p in bert_output_params)}")

param_groups = [
    {"params": bert_output_params, "lr": 3e-5, "weight_decay": 0.01},
...
ds_val = Subset(ds_base, [0, 7, 14, 21, 28, 35])
# ds_val_raw = PairsJSONLDataset(JSONL, ROOTDIR, image_size=SIZE, seg_mode="RGB")

# sanity check
print("[base] len:", len(ds_base))  # 应该是 1192

# REPEAT = 100            # 11 × 100 = 1100 条
# ds_train = RepeatDataset(ds_base, repeat=REPEAT)

# 3. 用缓存后的 ds_train 来建 DataLoader
dl     = DataLoader(ds_base, batch_size=BATCH, shuffle=False, num_workers=0, pin_memory=True)
dl_val = DataLoader(ds_val,   batch_size=6, shuffle=False, num_workers=0, pin_memory=True)
...
while global_step < STEPS:
    for batch in dl:
        ...
        if global_step % VAL_EVERY == 0:
            ...
            torch.save(lora_sd, os.path.join(OUTDIR, f"unet_lora_step8k_{global_step:06d}.pt"))
