...
OUTDIR  = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\batch_switch\expand_to_Q"
...
for n, p in model.model.diffusion_model.named_parameters():
    if (".to_k." in n or ".to_v." in n):
        p.requires_grad = False
print("[freeze] K/V LoRA frozen, only Q-LoRA trainable ✓")
...
ADAPTER_PT = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\batch_switch\textenc_adapter_step013000.pt"
LORA_PT    = r"D:\Junyhuang\Project2\Outputs_overfit\Sameseg_multiprompt\batch_switch\unet_lora_step013000.pt"
...
# === Stage-2.1: 轻解冻 adapter.final_ln + 更新优化器设定 ===
# 让 adapter 的最后一层 LayerNorm 可以重新平衡颜色分布
for n, p in textenc.adapter.named_parameters():
    if "final_ln" in n:
        p.requires_grad = True
        print(f"[unfreeze] adapter param trainable → {n}")

# 重新收集 trainable 参数
adapter_final_ln_params = [p for n, p in textenc.adapter.named_parameters() if p.requires_grad]
lora_q_params = [p for n, p in model.model.diffusion_model.named_parameters()
                 if p.requires_grad and ".to_q." in n and (".A.weight" in n or ".B.weight" in n)]

print(f"[check] q-lora params: {sum(p.numel() for p in lora_q_params)}")
print(f"[check] adapter.final_ln params: {sum(p.numel() for p in adapter_final_ln_params)}")

# === 新优化器 ===
param_groups = [
    {"params": lora_q_params, "lr": 2e-4, "weight_decay": 0.001},      # 主 Q-LoRA 学习率略降
    {"params": adapter_final_ln_params, "lr": 1e-5, "weight_decay": 0.0},  # LN 微调层，极小 lr
]
...
# === 重新定义 Warmup + Cosine 调度 ===
WARMUP = 14000        # 缩短 warmup，加快进入稳定阶段
TOTAL_STEPS = STEPS
